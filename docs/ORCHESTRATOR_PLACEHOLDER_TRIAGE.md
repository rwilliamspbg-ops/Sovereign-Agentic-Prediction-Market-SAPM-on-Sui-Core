...
## 2026-06-12 Provider Runtime Classification Update

Extended provider runtime telemetry in `agents/orchestrator/core/go-hybrid-provider.js`:

- added derive-session runtime state snapshot (`executionMode`, call count, timing, last error category)
- added explicit derive-session failure classification (`readiness_failed`, `invalid_json_response`, `invalid_payload`, `execution_failed`)
- added required response-field validation for provider payloads before session material is accepted

Extended discovery diagnostics in `agents/orchestrator/discovery/manager.js`:

- readiness success logs now include provider execution mode and derive call count
- readiness failure logs now include classified runtime error category

Added regression coverage:

- `agents/orchestrator/test/security-hardening.test.js` now validates runtime error classification and telemetry snapshot state
- `agents/orchestrator/test/discovery-manager.test.js` now validates readiness-failure classification propagation in discovery session failure path

Validation evidence:

- Command: npm --prefix agents/orchestrator test
- Result: 7 suites, 139 tests, 139 passed

## 2026-06-12 Provider Recovery Policy Update

Extended provider runtime behavior in `agents/orchestrator/core/go-hybrid-provider.js`:

- added bounded retry policy controls (`SAPM_HYBRID_KEX_MAX_RETRIES`, `SAPM_HYBRID_KEX_RETRY_BACKOFF_MS`)
- added retry-eligible error categories (`readiness_failed`, `execution_failed`, `timeout`) with readiness-cache reset between attempts
- added recovery telemetry (`deriveAttempts`, `recoveryAttempts`, `lastRecoveryAction`) for operator diagnostics

Added regression coverage in `agents/orchestrator/test/security-hardening.test.js`:

- transient provider failure now recovers on bounded retry with expected telemetry markers
- persistent provider failure now exhausts retries and fails closed with expected recovery counters

Validation evidence:

- Command: npm --prefix agents/orchestrator test
- Result: 7 suites, 141 tests, 141 passed

## 2026-06-12 Provider Lifecycle Controller Update

Extended provider lifecycle management in `agents/orchestrator/core/go-hybrid-provider.js`:

- added explicit lifecycle controller APIs: `startProviderLifecycle`, `healthCheckProviderLifecycle`, `stopProviderLifecycle`, `getProviderLifecycleState`
- added restart-budget window enforcement (`SAPM_HYBRID_KEX_RESTART_BUDGET_MAX`, `SAPM_HYBRID_KEX_RESTART_BUDGET_WINDOW_MS`)
- integrated derive-session recovery with lifecycle-aware restart accounting and restart-budget fail-closed semantics

Added regression coverage in `agents/orchestrator/test/security-hardening.test.js`:

- explicit lifecycle start/health/stop flow now has deterministic coverage
- restart-budget exhaustion path now fails closed with `restart_budget_exceeded` classification
