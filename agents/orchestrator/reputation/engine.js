...
class ReputationEngine {
  ...
  calculateAggregatedForecast(predictions, timestamp) {
    ...
    const minWeight = totalWeight * (1 - 2 * trimmedFactor)
    ...
    if (agentReputations.length > 3) {
      ...
      const keptCount = agentReputations.length - 2 * trimCount
      
      // Recalculate with trimmed agents
      let trimmedTotalWeight = 0
      let trimmedAggregatedScore = 0
      
      for (let i = trimCount; i < agentReputations.length - trimCount; i++) {
        const a = sorted[i]
        const weight = (a.reputation * a.confidence) / trimmedTotalWeight || 1
        trimmedTotalWeight += a.reputation * a.confidence
        trimmedAggregatedScore += this._normalizeScore(a.confidence) * weight
      }
      
      trimmedAggregatedScore /= (trimmedTotalWeight / totalWeight)
    }
